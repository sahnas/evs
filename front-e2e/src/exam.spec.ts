...
test('Affiche la liste des examens', async ({ page }) => {
  await page.goto('/');

  expect(await page.locator('app-exam-item').count()).toBeGreaterThan(0);
});

test('Affiche le message aucun examen si la liste est vide', async ({
  page,
}) => {
...
test('Création d’un nouvel examen', async ({ page }) => {
  await page.goto('/');
